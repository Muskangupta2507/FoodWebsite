...
// 	const alreadyExist = await users.findOne({ email: req.body.email },{isActive:true});
// 	if (alreadyExist) {
...
// 	try {
// 		const user = await users.findOne({ $and: [{ email: req.body.email }, { isActive: true }] }); // async, await used for promises. 
// 		if (user) {
